...
int CV_strip(){
  ...
  TCanvas *c0=new TCanvas("c0","Interstrip Capacitance @ 300 V",300,10,900,500);
//  c0->SetGrid();
  //TH1F *Hist= new TH1F("Hist","1D",127,0,127);
  TH1F *Hist= new TH1F("Hist","Interstrip Capacitance @ 300 V",127,0,126);
  ...
  if(pos!=-1)
  {
   ...
   vector<string> names;
    ...
    while(!inputFile.eof()){
      ...
      ||line[0]=='D'||line[0]=='C'||line[0]=='+'||line[0]=='V'|| line[0]==' '){continue;}
      else{
       stringstream ss;
       ss << line;
       ss >> Voltage;
        ...
        vector<double> rows;
        cout<<line<<endl;
      while (ss>>Capacitance)
          {

        //  ss >> Current;
          rows.push_back(Capacitance);
          cout<<ncols<<" : "<<Capacitance<<endl;
          ncols++;
          }
   ...
   gStyle->SetTitleAlign(23);
    ...
    for(int j=0; j<=Capacitances[0].size()-1; j++){
    ...
    //    Hist->GetYaxis()->SetNdivisions(127,0,0,kFALSE);
        Hist->SetBinContent(j+1,Capacitances[i][j]*1E+12);
      }
   //        cout<<name<<endl;
  ...
  //   Yaxis->SetNdivisions(1,kFALSE);
       ...
       Hist->SetTitle("Interstrip Capacitance at 300 V : Baby sensor : SN 11");
       Hist->GetXaxis()->SetTitle("Strip number");
       Hist->GetYaxis()->SetTitle("Capacitance [pF]");
