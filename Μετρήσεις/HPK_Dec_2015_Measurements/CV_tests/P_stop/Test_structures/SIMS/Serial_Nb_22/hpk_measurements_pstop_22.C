...
void hpk_measurements_pstop_22()
{ 
   ...
   gr4->SetMarkerStyle(23); gr4->SetMarkerColor(4);
     ...
     f1=new TF1("f1","pol1",0.0,180.0);
    f2=new TF1("f2","pol1",120.0,550);
...
//Circular_Diode.txt 
   ...
   while(getline(inputFile1, line)) {
    ...
    //   Backplane_Capacitance=Backplane_Capacitance*1E+15-2.54*1E+3;
     ...
     if(((abs(Voltage)>=115.0)&&(abs(Voltage)<=120.00))){continue;}
     else{
...
//Diode.txt
   ...
   //  gr1->SetPoint(points-1,abs(Voltage),Backplane_Capacitance);
     if(((abs(Voltage)>=119.0)&&(abs(Voltage)<=120.00))  ){continue;}
     else{
